...
(function(){
  // v2 spec: reduced-motion covers JS-driven animation, not just CSS.
  ...
  function resetThread(){
    if (reducedMotion()) return; // under reduce, never re-hide
  ...
  // Keep the observer attached; every re-entry resets and replays. Exit cancels timers.
  if ('IntersectionObserver' in window){
    ...
    if (_r.height > 0 && _vis / _r.height >= 0.32){ playing = false; playThread(); }
  } else {
    playThread();
  }
  ...
  // -- Animated stat counter: $500, re-arms on scroll re-entry, manual replay --
  // Stat: $500 = conservative anchor for one emergency AC service visit in NC.
  // Source: https://ainora.lt/blog/hvac-service-call-statistics-2026
  var STAT_TARGET = 500;
  ...
  if (statEl){
    ...
    var countRun = 0; // generation token: stale rAF loops self-cancel
  ...
  // Reduced motion first paint: SMS thread fully shown immediately
  if (reducedMotion()){ showThreadFinal(); }

  // -- A1: Sticky mobile CTA bar: hide when CTA section is in view --
